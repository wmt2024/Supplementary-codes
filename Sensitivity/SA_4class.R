...
df <- read_sav("R_ana.sav") %>%
  ...
  drop_na(SA, age, grade, gender, BDI, SAI, profile)

df$profile <- relevel(df$profile, ref = "4")

model1 <- glm(SA ~ age + grade + gender, data = df, family = binomial)
model2 <- glm(SA ~ age + grade + gender + BDI + SAI, data = df, family = binomial)
model3 <- glm(SA ~ age + grade + gender + BDI + SAI + profile, data = df, family = binomial)
